...
def part2(inp: str) -> str | int | None:
    devices = parse_input(inp)
    problematic_devices = {"dac", "fft"}

    @cache
    def dfs(node, target, required):
        if node == target:
            return 0 if required else 1

        required -= {node}
        return sum(dfs(n, target, required) for n in devices[node])

    return dfs("svr", "out", frozenset(problematic_devices))
